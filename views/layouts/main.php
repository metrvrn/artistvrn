...
<head>
    ...
    <meta name="yandex-verification" content="4fc77769a63cc18d" />
    <?= Html::csrfMetaTags() ?>
...
<div class="wrapper">
    ...
    $menuItems = [
        ...
        ['label' => 'Контакты ', 'url' => [$url = Url::to(['/site/contact'])]],
      //  ['label' => 'Каталог', 'url' => [$url = Url::to(['catalog/index',])]],
       // ['label' => 'Контакты ', 'url' => [$url = Url::to(['/site/contact'])]],
    ];
    if(LokalFileModel::getDataByKeyFromLocalfile('local_data_use_use_delivery')){
        $menuItems[] = ['label' => 'Доставка', 'url' => [Url::to(['/site/delivery'])]];
    }
    if(LokalFileModel::getDataByKeyFromLocalfile('local_data_use_about')){
        $menuItems[] = ['label' => 'О компании', 'url' => [Url::to(['/site/about'])]];
    }
